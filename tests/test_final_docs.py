...
def test_no_personal_identifier_email_or_local_path_is_tracked():
    ...
    for f in _tracked_files():
        if f.startswith("data/raw/") or f.endswith((".png", ".tar", ".webm", ".gif")) or f == "tests/test_final_docs.py":   # this file holds the patterns; binary files are not text
            continue
...
def test_no_build_phase_vocabulary_appears_anywhere_in_the_repository():
    ...
    problems = []
    for f in _tracked_files():
        if f.startswith("data/raw/") or f.endswith((".png", ".tar", ".svg", ".webm", ".gif")) or f == "tests/test_final_docs.py":   # binary files are not text
            continue
        p = REPO / f
        if not p.is_file():
            continue
