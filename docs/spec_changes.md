...
All changes below **have been applied** to the documents named. Reasons and evidence are in `docs/decision_log.md` (D17-D26) and `docs/profile_report.md`.
...
| Volume irregularity section: Nov 16-20 retained, flagged, not a data error; six flagged days, not one week | Nov 2 and Nov 6 also break the pattern; sessions on flagged days look ordinary | profile report section 8 |
| Population labels: registered-export / non-registered-export, inherited from filenames, not interpreted | source does not define them | the initial review and profiling |
...
| New table `fact_daily_volume` with `low_observed_volume_day`, `volume_irregularity`; days never excluded | volume flags need a home that cannot touch metrics | profile report section 7 |
...
| Thresholds file declared: `config/thresholds.yml` with the values in `validation_findings.md` §3 | thresholds set from the distribution | profile report section K |
| `core_ready` and M5 definition tightened; warn-free rate | keep weather and WARN rules out of readiness | see PRD |
| Tests added: row order, mixed timestamp formats, hour-ending weather join, volume flags never exclude, M4 within-session | each profiling finding becomes a regression test | n/a |
...
1. **Approve the proposed thresholds** in `docs/validation_findings.md` §3, especially B07 [50, 2,200] g and the C02 regime rule (a 4-week baseline hypothesis).
