...
*vs initial design: CHANGED (the initial design assumed one row per session)* · 2026-09-18
...
- **Alternatives tested.** (a) Row = session, as the initial design assumed: median "session" 63 g, P90 277 g, meaningless as a meal. (b) Group by `tray_id`: 687 trays, median 2,013 g, because 87% of trays are reused across days. (c) Group by `session_id` (chosen).
...
*vs initial design: CHANGED (the initial design quoted a checkout-scale total)* · 2026-09-18
...
*vs initial design: CHANGED (the initial design assumed the wrong source for the meal weight)* · 2026-09-18
...
- **Alternatives tested.** Reading the earlier claim as applying to the CSV: contradicted by the data.
...
*vs initial design: CHANGED (the initial design used a single timezone)* · 2026-09-18
...
*vs initial design: NEW* · 2026-09-18
...
*vs initial design: NEW* · 2026-09-18, reaffirmed 2026-09-19
...
*vs initial design: CONFIRMED* · 2026-09-18
...
*vs initial design: CHANGED (the initial design put weather inside readiness)* · 2026-09-18
...
## D9. Core KPI set
*vs initial design: CHANGED* · 2026-09-18, locked 2026-09-19
...
*vs initial design: CHANGED* · 2026-09-18
- **Decision.** This project establishes whether the measurements are sufficient for future waste decisions.
...
*vs initial design: CHANGED (the initial design had part 1 only)* · 2026-09-18
...
- **Why.** The second and third are what the initial review and profiling actually discovered.
- **Business impact.** Explains why the client should trust the pipeline's caveats.
- **Residual uncertainty.** None.

## D12. Weather station and retrieval
*vs initial design: NEW detail* · 2026-09-18
...
*vs initial design: CHANGED (the initial design differed)* · 2026-09-18
...
- **Alternatives tested.** Python 3.12+ and DuckDB (from the initial design) (unneeded).
- **Chosen approach.** Minimal standard stack.
- **Why.** Simplest architecture that proves the approach.
...
## D16. Placeholder numbers from the initial design are not used
*vs initial design: CONFIRMED as illustrative only* · 2026-09-18
...
*vs initial design: CHANGED* · 2026-09-19
...
*vs initial design: CHANGED (the initial review used floor)* · 2026-09-19
...
See D8 and D17. *vs initial design: CHANGED.* Recorded here to preserve numbering; no separate decision.

## D26. Corrections to the initial source review
*vs initial design: n/a* · 2026-09-19
...
- **Evidence.** The initial review pooled the crossover pairs across populations.
...
*vs initial design: NEW* · 2026-09-19, revised on review 2026-09-19
...
See D23. *vs initial design: NEW* · 2026-09-19. Recorded separately so the wording is not lost: no output describes a flagged value as impossible, erroneous or abnormal in general; it is "flagged by a threshold derived from this dataset".
...
- **Evidence.** 25 scenarios run in profiling: baseline (1); crossover inclusion (3); outlier and threshold exclusions including the 2,097 g removal (7); timezone hypotheses (5); volume diagnostics (4); definition tests (4); the diagnostic population (1).
...
*vs initial design: CHANGED (the initial design called it a data product / evidence pipeline)* · 2026-09-19
...
## D32. Repository structure for implementation: research/exploration, config/, golden fixtures
*vs initial design: NEW* · 2026-09-19
- **Decision.** The profiling investigation scripts move to `research/exploration/` (historical evidence, README, never imported by `src/`); production code lives in `src/`; judgement lives in `config/*.yml`; the profiling outputs are frozen as regression fixtures in `tests/golden/`.
...
*vs initial design: CHANGED (the initial design had retrieval inside the pipeline)* · 2026-09-19
...
- **Business impact.** The project reproduces on an aeroplane.
- **Residual uncertainty.** None.
...
*vs initial design: CHANGED (profiling stored only a normalised local time)* · 2026-09-19
...
*vs initial design: NEW* · 2026-09-19
- **Decision.** Every source row is staged: duplicates, zero or huge weights, unparseable timestamps and ragged rows are kept as found and marked with statuses. Deduplication, quarantine and thresholds belong to validation.
...
- **Business impact.** The profiling issue counts can be reproduced from staging.
- **Residual uncertainty.** None.
...
- **Residual uncertainty.** The checksum proves the table is what staging wrote, not that staging was right; staging tests cover that.
...
- **Alternatives tested.** Hard-coding severities in the rules (configuration and code could drift); renaming the profiling rule C02 (the specification already splits it into C02a and C02b).
...
## D49. The model consumes only verified inputs; validation outputs now record their checksums
*vs initial design: NEW* · 2026-09-19
- **Decision.** `src/model` reads the verified staging tables and the validation outputs, nothing else, and never opens `data/raw`. It refuses to build unless each validation file matches the SHA-256 recorded in `validation_summary.json`, that summary records the same staging checksums as the tables present, the disposition file lists exactly the staged events, and the quarantine manifest agrees with the summary. To make this possible validation now writes `output_sha256` into its summary (an additive change: no rule, threshold or output value changed).
- **Evidence.** Tests alter one byte of each validation file, delete the summary, point it at other staging checksums, drop an event from the dispositions, add an unknown disposition and remove a quarantine row; every case stops the model and removes stale canonical tables.
- **Alternatives tested.** Trusting the validation files without a checksum (a stale or edited file would be modelled silently); recomputing validation's decisions inside the model (would duplicate and could contradict the validation layer).
...
- **Residual uncertainty.** A consistent forgery of a validation file and its recorded checksum is caught only if it changes a figure the model recomputes independently (weights, spans, counts, dispositions); tests cover that.

## D50. The selected meal weight is reconstructed independently; validation's value is only a control
*vs initial design: NEW* · 2026-09-19
- **Decision.** `derived_selected_meal_weight_g` is the sum of `component_weight_g` over the session's MODELLABLE events, computed from `fact_weighing_event` rows; it is NULL (never 0) if there is no modellable event or any modellable weight is invalid. It is compared with validation's `rule_weight_sum_g` for every session. Any difference blocks the model, publishes no canonical rows, and is listed by session in `session_weight_control.csv`. A quarantined session has no canonical weight (its events stay, with their observed weights, marked QUARANTINED). The original wording "sum over non-duplicate events" is refined to MODELLABLE events, which is the same set for every non-quarantined session.
- **Evidence.** All 3,341 non-quarantined sessions match validation exactly and match an independent pandas recomputation; the four quarantined keys are EXCLUDED_QUARANTINED; a forged validation weight blocks the model and names exactly that session.
...
- **Business impact.** M1 and M2 can be computed in metrics from a value that has already been cross-checked.
- **Residual uncertainty.** The crossover-inclusion sensitivity scenario must rebuild those weights from the QUARANTINED event rows, which are all preserved.
...
*vs initial design: was deferred from validation* · 2026-09-19
...
- **Evidence.** 1,697 of 1,697 core-ready sessions match; the matched hours carry NULL `r_1h` for 67 sessions and NULL `ri_10min` for 25, as in profiling; the join hour equals an independent pandas ceiling of the first weighing.
- **Alternatives tested.** Flooring to the hour (profiling showed it mis-assigns `r_1h`); interpolating or backfilling NaN (invents data).
- **Chosen approach.** Join, flag, never fill.
- **Why.** Weather is context; a weather gap must not change a business figure.
- **Business impact.** None yet; no weather conclusion is drawn in model.
...
- **Alternatives tested.** A hand-written data dictionary (drifts); computing M5 in the model (a metric belongs to metrics).
...
- **Evidence.** Tests alter or delete each canonical table, the manifest and the validation run id; every case blocks the stage and removes stale metric files. A static check and a runtime file-open spy show metrics reads only `model/` and the validation summary. A forged session weight with a valid re-signed checksum is caught by the event-level control MC07 (one forged weight can leave the median and P90 within tolerance, so the controls matter).
- **Alternatives tested.** Recomputing weights from staging (model owns the derived field); mixing wording into the calculations (would let presentation change a number).
...
*vs initial design: NEW* · 2026-09-19
- **Decision.** Each contract stores its approved value and tolerance (from the profiling baseline and the golden values). A miss, a wrong population, or a wrong approved count makes the metric FAILED with the exact difference; the stage exits 4 and the outputs are still written so the discrepancy is visible. There is one statistical method per metric, documented in the contract (median; linear-interpolation P90).
...
- **Why.** The goal is evidence, not metric volume.
- **Business impact.** The final README can reuse the table as it stands.
- **Residual uncertainty.** None.

## D59. The baseline is frozen; scenarios are data; the profiling space is a hard reproduction gate
*vs initial design: NEW* · 2026-09-19
- **Decision.** The sensitivity analysis recomputes the baseline (S00) from the verified canonical tables and requires it to equal the approved package and the independent metrics computation. Every scenario is declared in `src/sensitivity/registry.py` (assumption changed, baseline and alternative values, rationale, tables, population, metrics, an `operation`, interpretation, defensibility, diagnostic-only flag, decision impact, and the approved reference) and run by a generic engine that knows operations, not scenarios. If the baseline moves or an approved reference stops reproducing, the stage FAILS (exit 4) with the difference; nothing is tuned.
- **Evidence.** All 25 reference scenarios reproduce (M1 and M2 within 0.05 g, counts exact), including the weather consequences of every offset. Tests show the analysis leaves the canonical files byte-identical and that overwriting the baseline, making +2h the production timezone, unquarantining the crossover sessions, excluding the irregular days, changing M5's denominator, adding a waste proxy or pooling populations are all rejected.
...
*vs initial design: NEW (extends the initial stage statuses)* · 2026-09-19
...
- **Alternatives tested.** Removing `data/raw/` and requiring an explicit fetch (rejected: weather would need re-pinning and change snapshot ids); Apache-2.0 (rejected as heavier than needed); rewriting history to change author metadata (rejected by the owner: history is preserved).
- **Chosen approach.** Keep the data, license only our own work, and keep drafting and course material out of the current tree (they remain in earlier commits, because history is preserved).
...
- **Residual uncertainty.** Earlier commits still contain the drafting plan and the course handout, and carry the author's original commit email; both are accepted by the owner for this submission.
