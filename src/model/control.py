"""Control checks: does the independently reconstructed canonical model agree with staging and with validation?

Validation's `rule_weight_sum_g` (and its span, counts and day table) are validation WORKING values. Here they are used only as a control:
...
Statuses as in validation reconciliation: PASS, WARN, FAIL, INFO.
"""
...
SESSION_CONTROL_COLUMNS = ("session_key", "population", "is_quarantined", "canonical_weight_g", "validation_rule_weight_sum_g", "weight_control",
                           "canonical_span_s", "validation_span_s", "span_control", "event_count_control", "canonical_first_weighing_local",
                           "validation_first_weighing_local", "first_weighing_control")
...
def session_control_rows(sessions: list[dict[str, Any]], inp: ModelInputs) -> list[dict[str, Any]]:
    rows = []
    for s in sessions:
        vstatus = inp.session_status[s["session_key"]]
        validation_weight, validation_span = _num(vstatus["rule_weight_sum_g"]), _num(vstatus["span_s"])
        if s["is_quarantined"]:
            weight_control = "EXCLUDED_QUARANTINED" if s["derived_selected_meal_weight_g"] is None else "MISMATCH"
        else:
            weight_control = "MATCH" if s["derived_selected_meal_weight_g"] == validation_weight else "MISMATCH"
        span_control = "MATCH" if s["session_span_s"] == validation_span else "MISMATCH"
        counts_ok = (s["event_count"] == int(vstatus["events_staged"]) and s["duplicate_excluded_event_count"] == int(vstatus["exact_repeats_excluded"])
                     and (s["is_quarantined"] or s["modellable_event_count"] == int(vstatus["events_counted"])))
        rows.append({"session_key": s["session_key"], "population": s["population"], "is_quarantined": s["is_quarantined"],
                     "canonical_weight_g": s["derived_selected_meal_weight_g"], "validation_rule_weight_sum_g": validation_weight, "weight_control": weight_control,
                     "canonical_span_s": s["session_span_s"], "validation_span_s": validation_span, "span_control": span_control,
                     "event_count_control": "MATCH" if counts_ok else "MISMATCH", "canonical_first_weighing_local": s["first_weighing_local"],
                     "validation_first_weighing_local": vstatus["first_weighing_local"] or None,
                     "first_weighing_control": "MATCH" if s["first_weighing_local"] == (vstatus["first_weighing_local"] or None) else "MISMATCH"})
    return rows
...
def control_checks(events: list[dict[str, Any]], sessions: list[dict[str, Any]], components: list[dict[str, Any]], weather: list[dict[str, Any]] | None,
    ...
    checks = [
        ...
        eq("M03", "events", "events by population equal validation", s4["events"]["by_population"], dict(sorted(Counter(e["population"] for e in events).items())), "validation summary"),
        eq("M04", "events", "dispositions equal validation (nothing dropped: modellable + duplicate-excluded + quarantined = staged)", s4["events"]["dispositions"], disp, "validation summary"),
        eq("M05", "sessions", "session keys equal validation and the sum of session event counts equals the event total", (s4["sessions"]["session_keys"], len(events)),
           (len(sessions), sum(s["event_count"] for s in sessions)), "validation summary"),
        eq("M06", "sessions", "distinct session ids equal the pinned count", f.expected_session_ids, len(ids), "pin: config/sources.yml"),
        eq("M07", "sessions", "session ids present in both populations equal validation (the crossover ids; keys are never merged)", s4["sessions"]["session_ids_in_both_populations"], both, "validation summary"),
        eq("M08", "sessions", "quarantined session keys equal the validation quarantine", sorted(inp.quarantined_keys), sorted(s["session_key"] for s in sessions if s["is_quarantined"]), "validation quarantine manifest"),
        eq("M09", "sessions", "every event of a quarantined key is QUARANTINED and no other event is", 0,
           sum((e["disposition"] == "QUARANTINED") != (e["session_key"] in inp.quarantined_keys) for e in events), "validation quarantine manifest"),
        eq("M10", "control", "sessions whose canonical selected-meal weight differs from validation rule_weight_sum_g (quarantined excluded by design)", [], mism("weight_control"), "independent reconstruction"),
        ...
        eq("M12", "control", "sessions whose canonical span differs from validation", [], mism("span_control"), "independent reconstruction"),
        eq("M13", "control", "sessions whose event, modellable or repeat counts differ from validation", [], mism("event_count_control"), "independent reconstruction"),
        eq("M14", "control", "sessions whose first weighing differs from validation", [], mism("first_weighing_control"), "independent reconstruction"),
        eq("M15", "components", "sessions where distinct raw and normalised component counts differ (profiling property)", [],
           sorted(s["session_key"] for s in sessions if s["distinct_raw_component_count"] != s["distinct_component_count"]), "approved profiling finding"),
        ...
        eq("M17", "timezone", "events carrying the file-specific override equal validation and sit only in the configured file(s)", 0,
        ...
        info("M19", "readiness", "core_ready sessions | eligible primary-population sessions (fields prepared for M5; the metric itself is metrics)",
    ...
    checks.append(eq("M21", "volume", "registered-export daily sessions and both flags equal the validation day table", True, same_days, "validation service_day_volume"))
    if weather is None:
        ...
        checks += [
            ...
            info("M25", "weather", "core_ready registered-export sessions matched | eligible (approved profiling: 1,697 of 1,697)",
